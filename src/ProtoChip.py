...
D = fd.Function(VD)
D.interpolate(fd.Constant(0.8),  subset=mesh.cell_subset(1))
D.interpolate(fd.Constant(0.4),  subset=mesh.cell_subset(2))
D.interpolate(fd.Constant(0.6),  subset=mesh.cell_subset(3))
...
# chip BCS
bcs = [                                
        #fd.DirichletBC(V, fd.Constant(CHIPT0),[5, 6, 8, 10]),
        ]
...
for i in range(300):
    ...
    a = cp1 * v  * dx \
            ...
            + h * D * alpha * cp1 * v  * fd.ds(49) 
    L = c0 * v  * fd.dx \
            + h * D * alpha * Cw * v  * fd.ds(49) \
            + h * v * processFunction(t+dt, tmax=13., sigma=1, C=80., offset=CHIPT0) * dx(2)\
            + h * v * processFunction(t+dt, tmax=20., sigma=0.5, C=40., offset=CHIPT0) * dx(3)\
            + h * v * processFunction(t+dt, tmax=7., sigma=1.2, C=30., offset=CHIPT0) * dx(3)
